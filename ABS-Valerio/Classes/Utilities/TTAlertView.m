...
//    self.progress = [[YLProgressBar alloc] initWithFrame:CGRectMake(10, 10, 100, 5)];
...
- (void)setButtonBackgroundImage:(UIImage *)image forState:(UIControlState)state atIndex:(NSUInteger)index
{
    ...
    rect.size.height = 30;
    [(UIButton *)[self.buttons objectAtIndex:index] setFrame:rect];
...
- (void)setButtonBackgroundImage:(UIImage *)image forState:(UIControlState)state withSize:(CGSize)size atIndex:(NSUInteger)index
{
    ...
    if (index == 0) {
        [(UIButton *)[self.buttons objectAtIndex:index]setTitleColor:[UIColor colorWithRed:255.0f/255.0f green:37.0f/255.0f blue:58.0f/255.0f alpha:1.0f] forState:UIControlStateHighlighted];
    }else{
        [(UIButton *)[self.buttons objectAtIndex:index]setTitleColor:[UIColor colorWithRed:255.0f/255.0f green:37.0f/255.0f blue:58.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
        [(UIButton *)[self.buttons objectAtIndex:index]setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
    }
...
- (void)layoutSubviews
{
    ...
    [self layoutInWindow:window];
    if (self.isProgress) {
        self.progress = [[YLProgressBar alloc] initWithFrame:CGRectMake(40, 55, 200, 10)];
        CGRect frame = self.messageScrollView.frame;
        frame.origin.y -= 10;
        [self.messageScrollView setFrame:frame];
        ...
        [self.containerView addSubview:self.progress];
    }
...
{
    NSArray *tintColors = @[[UIColor colorWithRed:255.0f/255.0f green:37.0f/255.0f blue:58.0f/255.0f alpha:1.0f]];
    self.progress.trackTintColor = [UIColor whiteColor];
    self.progress.type               = YLProgressBarTypeRounded;
    self.progress.progressTintColors = tintColors;
    self.progress.hideStripes        = YES;
    self.progress.behavior           = YLProgressBarBehaviorDefault;
}
