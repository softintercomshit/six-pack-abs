...
static NSString *const iTellAFriendiOSAppStoreURLFormat = @"http://itunes.apple.com/us/app/%@/id698154775?mt=8&ls=1";
static NSString *const iTellAFriendRateiOSAppStoreURLFormat = @"itms-apps://ax.itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?type=Purple+Software&id=698154775";
...
- (void)applicationLaunched
{
    ...
    if ([[defaults objectForKey:iTellAFriendAppKey] isEqualToString:applicationKey]) {
        ...
        ITELLLog(@"Icin Image: %@", self.appStoreIconImage);
...
{
    if (alertView==YES) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Gift This App", @"") message:[NSString stringWithFormat:NSLocalizedString(@"You really enjoy using %@. Your family and friends will love you for giving them this app.", @""), self.applicationName] delegate:self cancelButtonTitle:NSLocalizedString(@"cancelKey", @"") otherButtonTitles:NSLocalizedString(@"Gift", @""), nil];
        alertView.tag = 1;
        [alertView show];
...
- (NSString *)messageTitle
{
    if (messageTitle)
    {
        return NSLocalizedString(messageTitle, nil);
    }
    NSString *str = [NSString stringWithFormat:@"Check out %@", @"Six Pack ABS by VGFIT"];
...
//                                      "</table> \n"
//                                      "</div>",
//                                      NSLocalizedString(self.message, nil),
//                                      [self.appStoreURL absoluteString],
//                                      self.applicationName,
//                                      NSLocalizedString(@"Please note that you have not been added to any email lists.", nil)
//                                      ];NSLog(@"1Icon Image: %@", self.appStoreIconImage); 
//        
//        return emailBody;
//        
//    } 
...
- (NSURL *)appStoreURL
{
    ...
    return [NSURL URLWithString:[NSString stringWithFormat:iTellAFriendiOSAppStoreURLFormat, @"app %d", appStoreID]];
}
